...
export default function Settings() {
  ...
  const DEFAULT_REPLICATE_API_KEY =
    process.env.NEXT_PUBLIC_DEFAULT_REPLICATE_API_KEY ||
          ...
          if (settingsRes.ok) {
            ...
            if (settings.transcriptionConfig) {
              ...
              setTranscriptionConfig((prev) => ({
                ...
                ...config,
                cpuThreads:
                  config.cpuThreads || Math.min(2, systemInfo.cpuThreads || 1),
              ...
              if (config.mode === "replicate") {
                const apiKey =
                  config.replicateApiKey || DEFAULT_REPLICATE_API_KEY;
                const isDefaultKey =
                  !config.replicateApiKey || config.replicateApiKey === "";
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      ...
      if (
        ...
        !transcriptionConfig.replicateApiKey.trim()
      ) {
        toast.current?.show({
          severity: "info",
      ...
      if (res.ok) {
        ...
        if (transcriptionConfig.mode === "replicate") {
          const apiKey =
            transcriptionConfig.replicateApiKey || DEFAULT_REPLICATE_API_KEY;
          if (!apiVerified) {
            testApiKey(apiKey);
          }
        }
  ...
  const testApiKey = async (apiKey: string, silent = false) => {
    ...
    try {
      ...
      if (testResult.success) {
        ...
        if (!silent) {
          toast.current?.show({
            ...
            detail: isDefaultKey
              ...
              : testResult.message ||
                "Your Replicate API key is valid and working correctly",
            life: 5000,
          });
        }
      } else {
        ...
        if (isDefaultKey) {
          setApiError(
            "System default API key is currently unavailable. Network issue or key expired."
          );
        } else {
      ...
      if (isNetworkError) {
        setApiError(
          "Network error. Unable to connect to Replicate API service."
        );
      } else {
  ...
  const ModelSizeDescription = ({ size }: { size: string }) => {
    const descriptions: Record<
      string,
      { desc: string; specs: string; recommended: string }
    > = {
      tiny: {
    ...
    return (
      <div className="model-size-info">
        <p>{info.desc}</p>
        <p>
          <strong>Requirements:</strong> {info.specs}
        </p>
        <p>
          <strong>Recommended for:</strong> {info.recommended}
        </p>
      </div>
    );
  };
      ...
      >
        <TabPanel header="Transcription Settings">
          <div className="p-grid settings-grid">
            ...
            <div className="card">
              ...
              {transcriptionConfig.mode === "replicate" && (
                <div className="replicate-info">
                  <Message
                    severity="info"
                    text="Replicate provides cloud-based transcription using OpenAI's Whisper models. No local GPU required."
                  />
                </div>
              )}
            </div>
            ...
            {transcriptionConfig.mode === "local-gpu" &&
              systemInfo.gpuCount > 0 && (
                <div className="card">
                  <h3>GPU Settings</h3>
                  <div className="field">
                    ...
                    <div className="radio-group">
                      {Array.from({ length: systemInfo.gpuCount }).map(
                        (_, idx) => (
            ...
            {transcriptionConfig.mode === "replicate" && (
              <div className="card">
                ...
                <div className="field">
                  ...
                  <div className="p-inputgroup">
                    ...
                    <Button
                      ...
                      className={
                        apiVerified ? "p-button-success" : "p-button-primary"
                      }
                      disabled={apiKeyTesting}
                  ...
                  {apiVerified ? (
                    <Message
                      ...
                      text={`API key verified ${
                        apiUsername ? `for user ${apiUsername}` : ""
                      }`}
                    />
                  ) : apiError ? (
                    ...
                    <small className="p-text-secondary">
                      You can use your own API key or leave empty to use the
                      system default key.
                      <a
                ...
                <div className="field mt-4">
                  ...
                  />
                  <small className="p-text-secondary">
                    A good prompt can improve transcription accuracy for
                    domain-specific content, accents, or technical terminology.
                  </small>
                </div>
              </div>
            )}
        ...
        <TabPanel header="Help & Documentation">
          <div className="help-container">
            <Card
              title="Transcription Settings Help"
              className="documentation-card"
            >
              <h4>Transcription Methods</h4>
              <ul>
                <li>
                  <strong>Local CPU</strong>: Transcribes audio using your
                  computer's processor. Good for most users but may be slower
                  for longer files.
                </li>
                <li>
                  <strong>Local GPU</strong>: Uses your graphics card to
                  accelerate transcription. Much faster than CPU but requires a
                  compatible NVIDIA GPU.
                </li>
                <li>
                  <strong>Replicate API</strong>: Cloud-based transcription
                  using OpenAI's Whisper models. No local GPU required, works on
                  any device but requires an internet connection and API key.
                </li>
              </ul>

              <h4>Model Sizes</h4>
              <p>
                Whisper models come in different sizes, with larger models
                offering better accuracy but requiring more time and system
                resources:
              </p>
              <ul>
                <li>
                  <strong>Tiny</strong>: Fastest but least accurate
                </li>
                <li>
                  <strong>Base</strong>: Good balance for simple content
                </li>
                <li>
                  <strong>Small</strong>: Recommended default for most uses
                </li>
                <li>
                  <strong>Medium</strong>: High accuracy, good for challenging
                  audio
                </li>
                <li>
                  <strong>Large</strong>: Highest accuracy, requires significant
                  resources
                </li>
              </ul>

              <h4>Replicate API</h4>
              <p>To use the Replicate cloud-based transcription:</p>
              <ol>
                <li>
                  You can use the system default API key by leaving the field
                  empty
                </li>
                <li>
              ...
              <p>
                Replicate provides free credits for new accounts, and their
                usage-based pricing is reasonable for occasional transcription.
              </p>

              <h4>Transcription Prompts</h4>
              <p>
                When using Replicate API, you can provide optional prompts to
                guide the transcription. Good prompts include:
              </p>
              <ul>
                <li>
                  Domain knowledge: "This is a medical lecture about cardiology
                  with technical terms"
                </li>
                <li>Accent information: "The speaker has a Scottish accent"</li>
                <li>
                  Content structure: "This is a podcast interview with two
                  speakers"
                </li>
              </ul>
            </Card>
            ...
            <Card title="Troubleshooting" className="documentation-card">
              ...
              <ul>
                <li>
                  If Socket.IO shows as disconnected, real-time updates won't
                  work. Try refreshing the page.
                </li>
                <li>
                  For persistent connection issues, ensure both frontend and
                  backend are running.
                </li>
              </ul>
              ...
              <ul>
                <li>
                  If your API key fails verification, check that you've copied
                  it correctly.
                </li>
                <li>
                  API keys start with "r8_" and should not include any spaces.
                </li>
                <li>
                  If transcription fails, check your account's API token status
                  and credits.
                </li>
                <li>
                  If the system default key fails, try using your own API key
                  instead.
                </li>
              </ul>
              ...
              <ul>
                <li>
                  If GPU transcription fails, make sure you have an NVIDIA GPU
                  with CUDA support.
                </li>
                <li>
                  For memory errors, try a smaller model size or reduce CPU
                  threads.
                </li>
                <li>
                  Very long audio files may fail with larger models due to
                  memory constraints.
                </li>
              </ul>
            </Card>
