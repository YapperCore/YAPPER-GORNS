...
def transcribe_audio(audio_path, model_name=WHISPER_MODEL):
    ...
    try:
        ...
        model = whisper.load_model(
            model_name, device="cuda" if CUDA_AVAILABLE else "cpu"
        )
        logger.info(
            f"Model loaded successfully on {'GPU' if CUDA_AVAILABLE else 'CPU'}"
        )
...
def chunked_transcribe_audio(audio_path, model_name=WHISPER_MODEL, chunk_size=30):
    ...
    try:
        ...
        logger.info(
            f"Audio duration: {audio_duration:.2f}s, total chunks: {total_chunks}"
        )

        for i in range(total_chunks):
            start_sample = i * chunk_size * whisper.audio.SAMPLE_RATE
            end_sample = min(
                len(audio), (i + 1) * chunk_size * whisper.audio.SAMPLE_RATE
            )
