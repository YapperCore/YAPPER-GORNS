    ...
    if not firebase_admin._apps:
        try:
            ...
            if not os.path.exists(FIREBASE_SERVICE_ACCOUNT_KEY):
                logger.error(
                    f"Firebase service account key not found: {FIREBASE_SERVICE_ACCOUNT_KEY}"
                )
                return False
...
def upload_file(
    file_data, original_filename, user_id, content_type="application/octet-stream"
):
        ...
        doc_ref.set(
            {
                ...
                + datetime.timedelta(days=7),  # Store as datetime directly
                "expiresAtString": (
                    datetime.datetime.now() + datetime.timedelta(days=7)
                ).isoformat(),  # Use string as backup
...
def get_file_url(storage_path, requesting_user_id=None):
    ...
    try:
        ...
        if (
            path_owner
            and requesting_user_id != path_owner
            and not is_admin(requesting_user_id)
        ):
            logger.warning(
                f"Access denied: User {requesting_user_id} is not owner of {storage_path}"
            )
            return None
...
def delete_file(storage_path, requesting_user_id):
    ...
    try:
        ...
        path_owner = extract_owner_from_path(storage_path)
        if path_owner and (
            path_owner == requesting_user_id or is_admin(requesting_user_id)
        ):
            # Delete from Storage
            blob.delete()
            ...
            logger.warning(
                f"Delete denied: User {requesting_user_id} is not owner of {storage_path}"
            )
            return False
    except Exception as e:
...
def move_file(source_path, dest_path, requesting_user_id):
    ...
    try:
        ...
        if not source_blob.exists():
            ...
            if dest_blob.exists():
                logger.info(
                    f"Destination already exists: {dest_path}, treating as success"
                )
                ...
                if doc.exists():
                    ...
                    doc_ref.update({"storagePath": dest_path, "status": status})
        ...
        if source_owner and dest_owner and source_owner == dest_owner:
            if source_owner == requesting_user_id or is_admin(requesting_user_id):
                ...
                logger.warning(
                    f"Move denied: User {requesting_user_id} doesn't match owner {source_owner}"
                )
                return False
        else:
            logger.warning(
                f"Move denied: Source owner {source_owner} and destination owner {dest_owner} don't match"
            )
            return False
    except Exception as e:
...
def list_user_files(user_id, folder_path):
    ...
    try:
        ...
        if not files:
            ...
            logger.debug(
                f"Files retrieved for user {user_id} in {folder_path}: {files}"
            )

        return files
    except Exception as e:
        logger.error(
            f"Error listing user files for user {user_id} in folder {folder_path}: {e}"
        )
        return []
...
def verify_token(id_token):
    ...
    try:
        ...
        return {"uid": user_id, "email": email, "is_admin": is_admin(user_id)}
    except Exception as e:
