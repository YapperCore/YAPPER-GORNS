...
def handle_audio_position(data):
    ...
    if doc_id and position is not None:
        ...
        emit("audio_position", {"doc_id": doc_id, "position": position}, room=doc_id)
...
def emit_partial_transcript(doc_id, chunks, segments=None):
    ...
    try:
        ...
        socketio.emit(
            "partial_transcript_batch",
            {"doc_id": doc_id, "chunks": chunks, "segments": segments or []},
            room=doc_id,
        )
...
def emit_final_transcript(doc_id):
    ...
    """
    try:
        socketio.emit("final_transcript", {"doc_id": doc_id, "done": True}, room=doc_id)
