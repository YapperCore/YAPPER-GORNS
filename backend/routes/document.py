    ...
    try:
        ...
        try:
            ...
            if is_replicate:
                ...
                logger.info(f"Using API key (first 5 chars): {api_key[:5] if api_key else 'None'}")
...
def background_replicate_transcription(file_path, doc_id, user_settings):
    try:
        ...
        prompt = user_settings.get("transcriptionPrompt", "").strip()
        # Default to "transcribe" if prompt is empty
        if not prompt:
            prompt = "transcribe"
            logger.info("Using default prompt: 'transcribe'")
        ...
        logger.info(f"Calling transcribe_with_replicate for document {doc_id}")
