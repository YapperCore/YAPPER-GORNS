...
from transcribe import chunked_transcribe_audio, transcribe_with_replicate
...
def upload_audio():
    ...
    try:
        ...
        try:
            ...
            # IMPORTANT FIX: Force using Replicate if set in user settings
            if is_replicate:
                logger.info(f"Starting Replicate transcription for document {doc_id}")
                user_settings_for_job = {
                    "replicateApiKey": transcription_config.get("replicateApiKey", ""),
...
def transcribe_document(doc_id):
    try:
        ...
        # IMPORTANT FIX: Ensure we get the correct is_replicate value
        is_replicate = transcription_config.get("mode") == "replicate"
        ...
        doc["content"] = ""
        doc["is_replicate"] = is_replicate  # Ensure this is set correctly
        save_doc_store()
        ...
        if is_replicate:
            ...
            logger.info(f"Starting Replicate transcription for document {doc_id} with API key: {user_settings_for_job['replicateApiKey'][:5]}...")
            socketio.start_background_task(
                background_replicate_transcription, 
                file_path, 
                doc_id,
...
def background_replicate_transcription(file_path, doc_id, user_settings):
    try:
        ...
        # IMPORTANT FIX: Ensure API key is correctly passed
        ...
        logger.info(f"Received transcription from Replicate: {len(final_text)} characters")
        
        # Provide progress updates along the way
        socketio.emit("partial_transcript_batch", {
            "doc_id": doc_id,
            "progress": 70,
            "is_replicate": True,
